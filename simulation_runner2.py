...
class simulation_runner(object):
    ...
    def measure_terrains(self):
        ...
        if self.reward_function == 'Testing Proxy' or self.reward_function == 'Recorded Simulation':
            ...
            # collisionFilterMask : only test hits if the bitwise and between
    ...
    def load_robots(self,robot_name, randomize_xyyaw=True, start_xyyaw= None):
        ...
        if is_valid and self.reward_function == 'Simulation':
            ...
            self.module_action_len= list(np.diff(envs[0].action_indexes))
            self.attachments = envs[0].attachments
            self.n_modules = len(envs[0].modules_types)
            self.pos_queues = [deque(maxlen=20)]*num_envs
    ...
    def start_video(self, video_name_addition):
        self.logID = None
        env = envs[0]
        if env.show_GUI and self.record_video:
            vid_path = os.path.join(cwd, 
                        env.loaded_urdf+ video_name_addition+'.mp4')

            if not os.path.exists(vid_path):
                self.logID = env.p.startStateLogging(
    ...
    def simulate_robot(self, 
        video_name_addition, 
        n_time_steps=150, debug_params = None):
        ...
        for step in range(n_time_steps):
            out = self.step_simulation(debug_params, robot_alive)
        ...
        for i_env in range(num_envs):
            ...
            if env.show_GUI and (self.logID is not None):
    ...
    def step_simulation(self, debug_params = None, robot_alive=[True]):
        ...
        for i_env in range(num_envs):
            if robot_alive[i_env]:
                ...
                if np.abs(env.pos_xyz[1])>self.max_xy[1]:
                    robot_alive[i_env] = False
        ...
        if debug_params is not None:
            param_now = envs[0].p.readUserDebugParameter(debug_params[0])
            if not(param_now == initial_debug_params[0]):
