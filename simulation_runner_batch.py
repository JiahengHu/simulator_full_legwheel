...
class simulation_runner(object):
    ...
    def __init__(self, num_envs = 2):
        ...
        self.terrain_grid = []
        self.max_xy = [7,2] # defines size of obstacle course. TODO: get from terrain randomizer.
        ...
        self.show_GUI = False # if true, will show first gui. for test only.
        ...
        self.pool  = torch.multiprocessing.Pool(processes=1)
    ...
    def measure_terrains(self):
        ...
        if self.reward_function == 'Testing Proxy' or self.reward_function == 'Recorded Simulation':
            ...
            #         terrain_grid[i_env] += 1 # make terrain entries large sometimes for test
    ...
    def randomize_terrains(self,
        ...
        if self.reward_function == 'Simulation':
            ...
            for i_env in range(self.num_envs):
                ...
                # self.terrain_randomizer.reset(self.envs[i_env].p)
    ...
    # todo: set up a batch pool to run simulations with runner
                ...
                # print('starting plan_batch pool')
        ...
        if self.reward_function == 'Simulation':
            ...
            for i_env in range(self.num_envs):
                if not(self.is_valid[i_env]):
                    ...
                    env = self.envs[i_env]
                    self.envs[i_env].sim_speed_factor = 2
                    modules = self.modules_list[i_env]

                    pool_inputs.append( [i_env, rewards,
                            self.envs[i_env], 
            ...
            if len(pool_inputs)>0:               
                self.pool.starmap(simulate_robot, pool_inputs)
            ...
            # alter rewards based on terrain
                if torch.mean(self.terrain_grid[i_env])>0.5:
                    w_reward = 1
                    l_reward = 2
                else:
                    l_reward = 1
...
def simulate_robot(i_env, rewards, env, modules, record_video, max_xy):
    n_time_steps=150
    module_action_len= list(np.diff(env.action_indexes))
    attachments = env.attachments
